...
# Λίστα κόμβων για διακλάδωση (value, bounds, επιπλέον περιορισμοί)
nodes = [(-best_value, bounds, A, b, best_solution)]
final_solution = None
final_value = -np.inf
...
while nodes:
    ...
    current_value, current_bounds, current_A, current_b, current_solution = nodes.pop()
    ...
    for i in range(len(current_solution)):
        if not (current_solution[i] == 0 or current_solution[i] == 1):
            ...
            # Προσθήκη επιπέδων αποκοπής
            if i == 2:  # αν xi = x3
                new_A = np.vstack([current_A, [1, 0, 1, 0, 0]])
                new_b = np.hstack([current_b, [1]])
                val_zero, sol_zero = solve_knapsack(c, new_A, new_b, new_bounds_zero)
            else:
                val_zero, sol_zero = solve_knapsack(c, current_A, current_b, new_bounds_zero)

            if val_zero is not None:
                val_zero = -val_zero
                if val_zero > final_value:
                    nodes.append((-val_zero, new_bounds_zero, new_A, new_b, sol_zero))

            val_one, sol_one = solve_knapsack(c, current_A, current_b, new_bounds_one)
            if val_one is not None:
                val_one = -val_one
                if val_one > final_value:
                    nodes.append((-val_one, new_bounds_one, current_A, current_b, sol_one))
            break
